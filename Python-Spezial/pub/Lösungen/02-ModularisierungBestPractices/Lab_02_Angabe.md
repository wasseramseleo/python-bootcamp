...
Das Skript aus Lab 01 funktioniert, ist aber "Spaghetti-Code". Andere Abteilungen der PyBank möchten Ihre
Umrechnungslogik nutzen. Wir müssen den Code aufräumen, Logik in Funktionen kapseln und robuster machen.
...
Ziel ist die Kapselung der Logik in wiederverwendbare Funktionen mit klaren Schnittstellen (Type Hints), wie im
Theorie-Beispiel `calculate_condition_index`.
...
3. **Zeitstempel:**
    ...
    * Erstellen Sie eine kleine Hilfsfunktion `get_timestamp()`, die den aktuellen Zeitpunkt als String (ISO Format)
      zurückgibt.
...
Die echte Welt liefert oft "schmutzige" Daten (z.B. Beträge als String mit Tippfehlern). Ziel ist die Absicherung gegen Abstürze bei fehlerhaften Daten.
